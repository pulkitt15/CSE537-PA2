...
def get_lis(k,pt,key):                    # call the encrypt function and store outputs of rounds 1 to 16(/IP-1 since the
    lis=[]                            #swap and inverse permutation do not change the hamming distance)
...
def get_plot(lis,name,increment,initial):
    ...
    for i,li in enumerate(compare):
        temp=[]
        temp.append(x)
        x+=increment                          # store output of round 0 (plaintext/P since permutation does not change hamming distance)
    ...
    for j in range(len(hamming[0])-1):
        ...
        for i in range(len(hamming)):
            temp.append(hamming[i][j])            # calculate hamming distances between outputs of various round
